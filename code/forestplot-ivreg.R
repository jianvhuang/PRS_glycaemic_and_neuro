...
######################################################
# order and reference
df4plot[,group4facet:=paste0(exposure)]
...
######################################################
# labelling
list.facet.x=c("HOMA-IR","Fasting glucose")
names(list.facet.x)=c("CBH2226HOIR_z","CGL2226FAST_z")
...
if (nSize==2 & nColour==2){
  forest <- ggplot(data=df4plot.order,aes(x=tzvalue.robust,y=yRef))+
    facet_grid(~ exposure,  labeller = labeller(exposure = list.facet.x)) +
  ...
  ggsave(plot=forest,
         filename=paste0(DIR.output,"/ForestPlot-sample-",strata.temp,"-ivreg.png"), 
         width = 20, height = 10, units="cm",device='png', dpi=300)
}
########################################
